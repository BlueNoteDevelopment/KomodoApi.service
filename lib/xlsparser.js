        ...
        if (worksheet) {
            ...
            if(table){
                if(table.Rows.length===1 && table.Cols.length===1 && extractOptions.noHeader ){
                    ...
                    var targetCell = worksheet[XLSX.utils.encode_cell(anchorCell)];
                    table.Cols[0].Name= XLSX.utils.format_cell(targetCell);
                }
                return table;
            }
    ...
    function extractDataBrickToTable(workbook,options) {
        ...
        if (worksheet) {
            ...
            if (options.endCol === 0) {
                ...
                //XLSX is 0 based
                h_row = options.headerRow === 0 ? options.startRow - 1 : options.headerRow - 1;
                var h_col = options.startCol - 1;
                do {

                    c = worksheet[XLSX.utils.encode_cell({r: h_row, c: h_col})];
                    if (c) {
                        endValue = c.v;
                    } else {
                        endValue = '';
            ...
            for (h = options.startCol-1; h < options.endCol; h++) {
                c = worksheet[XLSX.utils.encode_cell({r: h_row, c: h})];
            ...
            for (var r = _row; ; r++) {
                var data = [];
                for (h = options.startCol-1; h < options.endCol; h++) {
                    var cell = worksheet[XLSX.utils.encode_cell({r: r, c: h})];
                    if (cell) {
                        data.push (options.preserveFormat ? XLSX.utils.format_cell(cell).trim():cell.v);
                    } else {
                        data.push('');
                    }
                }
