...
//    var _notifcationCallback = notificationCallback;
    ...
    function startSchedule(callback) {
        ...
        if(self.scheduler){self.scheduler = require('node-cron');}
        
        if(self.CollectionConfig.schedule){
            if (self.CollectionConfig.schedule.trigger.toLower() === 'interval'){
                if(self.CollectionConfig.schedule.interval<= 0 ){self.CollectionConfig.schedule.interval=5;}
                
                                
                self.scheduler.schedule('*/' + self.CollectionConfig.schedule.interval + ' * * * *', self.Execute((error, success) => {
                    startSchedule((e, s) => {
            ...
            }else if (self.CollectionConfig.schedule.trigger.toLower() === 'weekly'){
                
            }else if (self.CollectionConfig.schedule.trigger.toLower() === 'monthly'){
                
            }else{
                //self.CollectionConfig.schedule.trigger.toLower() === 'manual'
            }
        }
