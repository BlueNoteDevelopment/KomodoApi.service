...
//    var _notifcationCallback = notificationCallback;
    ...
    this.Start = function(callback){
        ...
        try{
            ...
            if(self.CollectionConfig.dataSourceType === 'xls' || self.CollectionConfig.dataSourceType === 'csv' ){
                ...
                startSchedule((error,success)=>{
                            if(error){
                                self.NotificationCallBack('EXECFATALERROR', {error: error,exec: self , operation:'failed to set schedule ' +  self.CollectionConfig.name  });
                                return callback(error,false,this);
                            }else{
                                self._isFolderWatch = false;
                                return callback(null,true,this);
    ...
    this.Execute = function(callback){
        ...
        //
            //theoretically, this should restart task
            return callback(null, true, this);
    ...
    function startSchedule(callback) {
        ...
        if (!self.scheduler) {
            self.scheduler = require('node-cron');
        }

        try {
            if (self.CollectionConfig.schedule) {
                if (self.CollectionConfig.schedule.trigger.toLower() === 'interval') {
                    if (self.CollectionConfig.schedule.intervalMinutes <= 0) {
                        self.CollectionConfig.schedule.intervalMinutes = 5;
                    }
                    if (self.CollectionConfig.schedule.intervalHours <= 0) {
                        self.CollectionConfig.schedule.intervalHours = 0;
                    }

                    if (self.CollectionConfig.schedule.intervalHours > 0) {
                        crontab = '* */' + self.CollectionConfig.schedule.intervalHours + ' * * *';
                    } else if (self.CollectionConfig.schedule.intervalMinutes > 0) {
                        crontab = '*/' + self.CollectionConfig.schedule.intervalMinutes + ' * * * *';
                    }

                } else if (self.CollectionConfig.schedule.trigger.toLower() === 'daily' ||
                        self.CollectionConfig.schedule.trigger.toLower() === 'monthly') {
                    ...
                    if (self.CollectionConfig.schedule.trigger.toLower() === 'daily') {
                    ...
                    if (self.CollectionConfig.schedule.trigger.toLower() === 'monthly') {
                ...
                self.scheduler.schedule(crontab, self.Execute((error, success) => {
                    self.scheduler.stop();
    ...
    function postFileOperation(originalFile,processedFile,isError,operation,moveToFolder,renameExtention,callback){
        ...
        if(isError){
            ...
            var moveToFolder = './error';
            var pathInfo = path.parse(originalFile);
            var moveToPath = '';
            ...
            var pathInfo = path.parse(moveToPath);
            ...
            if(operation.toLowerCase() === 'delete'){
                ...
                if(moveToFolder==='' || moveToFolder === "." || moveToFolder==='./'  || moveToFolder==='.\\'
